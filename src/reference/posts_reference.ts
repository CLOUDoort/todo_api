...
router.put('/users/:userIdx/posts/:postIdx', async (request: Request, response: Response) => {
    ...
    try {
        ...
        const countUser = await connection.run(`SELECT COUNT(*) AS count FROM users WHERE idx = ?;`, [userIdx])
        if (countUser[0].count !== 1) {
            throw new Error('해당 유저는 존재하지 않습니다.')
        }
        const countPost = await connection.run(`SELECT COUNT(*) AS count FROM posts WHERE idx = ?;`, [postIdx])
